...
basedir = '/export/home/extragal/francesco.chititegli/TIGRESS_1/History_SPExpanded_LGR4/lucia/'
...
output_dir_exp_history = '/export/home/extragal/francesco.chititegli/TIGRESS_1/History_SPExpanded_LGR4/'
...
snapshots_times_to_export =  [t // 2 for t in s.nums[beginning:beginning+10]]
...
for snapshot_number in snapshot_indexes_to_export:
    print('Snapshot number ' + str(snapshot_number) + ' at time ' + str(s.nums[snapshot_number] // 2) + ' Myr')
    s = pa.LoadSim(basedir, verbose=False)
    ds = s.load_vtk(num=s.nums[snapshot_number])
    Lx1, Ly1, Lz1 = ds.domain["Lx"]
    dx1, dy1, dz1 = ds.domain["dx"]
    
    qOmL = s.par["problem"]["qshear"] * s.par["problem"]["Omega"] * Lx1
    time = ds.domain["time"]
    qOmLt = qOmL * time
    volume = dx[0] * dx[1] * dx[2]
    ...
    for dx_shift in [-L_w[0], 0., L_w[0]]:
        for dy_shift in [-L_w[0], 0., L_w[0]]:
            ...
            shifted_pos[:, 1] = ((shifted_pos[:, 1] + half_box_size) % box_size) - half_box_size
    ...
    mask_inside_box = (
    (positions_stars_exp_unmasked[:, 0] >= -768) & (positions_stars_exp_unmasked[:, 0] <= 768) &
    (positions_stars_exp_unmasked[:, 1] >= -768) & (positions_stars_exp_unmasked[:, 1] <= 768))
    ...
    for i in np.arange(positions_stars_exp.shape[0]):
        if (min_height * 2 - 1536 <= positions_stars_exp[i,2]):
            if (positions_stars_exp[i,2] < max_height * 2 - 1536):
                counter = counter + 1
...
for indx, snapshot_number in enumerate(snapshot_indexes_to_export):
    ...
    for i in range(Nx_exp[0]):
        ...
        for j in range(Nx_exp[1]):
            for k in range(min_height, max_height):
                ...
                H_surf_val = hydrogen_density_val * (units.g).to(units.M_sun) * number_density_cf * 2.0
    ...
    print('Total mass analitically computed: ' + str(9 * 512 * 512 * 42 * 512 / np.sqrt(768**2 + 245**2)) + ' Msun')
    ...
    print('Difference An - Num: ' + str((9 * 512 * 512 * 42 * 512 / np.sqrt(768**2 + 245**2)) - (np.mean(old_disk_array[:,6]) * (Nx[0] * Nx[1] * (max_height-min_height) * 9))) + ' Msun')
    print('Average metallicity: ' + str(np.mean(old_disk_array[:,7])))

    #######################################################################################################################

    true_output = output_dir_exp_history + 'SnapLG' + str(snapshot_number) + '/'
    ...
    image_HI = np.sum((hydrogen_array[:, 10]).reshape(768, 768, tot_height_cells), axis=-1).transpose()[256:512,256:512]

    hdu = fits.PrimaryHDU(image_HI)
    hdu.header['BUNIT'] = 'erg/s'
    name_comment = 'HI Flux Density, SnapLG ' + str(snapshot_number)
